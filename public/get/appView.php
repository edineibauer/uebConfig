...
function getCachedContent(string $path, array $dados): array
{
    ...
    if (file_exists(PATH_HOME . "{$path}/view")) {
        foreach (Helper::listFolder(PATH_HOME . "{$path}/view") as $view) {
            ...
            $notReload = ["updateSystem.php", "403.php", "404.php", "cadastro-usuario.php", "esqueci-a-senha.php", "formulario.php", "inserir-nova-senha.php", "listagem.php", "login.php", "logout.php", "network.php"];

            if (preg_match('/\.php$/i', $view) && !in_array($viewUrl, $dados['view']) && !in_array($view, $notReload))
                $dados['view'][] = $viewUrl;
        }
    }
